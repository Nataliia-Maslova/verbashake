...
from engine.recommender import DIFFICULTY_TO_CEFR, LANG_TO_CODE
...
def seed_target_grammar_lessons() -> list[dict]:
    """
    engine.target_grammar_paths topics promoted to real Grammar-module
    lessons (2026-08-23, Natalia's request -- "Ukrainian cases should show
    up inside Nouns & Quantities like any other lesson", not live only in
    the Phase 3 practice picker). unit_id uses the SAME "grammar:<lesson_id>"
    scheme as seed_grammar() above (lesson_id >= 1000, see
    engine/target_grammar_paths.py's module docstring for why that range is
    permanent/never-renumber) -- one shared module, so mastery/SRS/recommender
    treat these exactly like any of the original 182 lessons.

    source_lesson=<lang_code> (not the numeric lesson_id, unlike
    seed_grammar()'s real lessons) is what locks each of these to its own
    one target language via engine.recommender._candidates()'s existing
    "reading"/"target_grammar" mechanism -- a Ukrainian-cases lesson must
    never be offered to a Spanish learner. Only topics that actually have
    batch-generated sentences (data/target_grammar_drills.csv) are seeded,
    same guard as seed_target_grammar() below -- a content_units row with
    nothing to render would be worse than not existing.
    """
    drills_path = DATA_DIR / "target_grammar_drills.csv"
    if not drills_path.exists():
        return []
    with open(drills_path, encoding="utf-8", newline="") as f:
        generated = {(row["lang"], row["topic_key"]) for row in csv.DictReader(f)}

    from engine.target_grammar_paths import TARGET_GRAMMAR_PATHS
    ...
    for lang, topics in TARGET_GRAMMAR_PATHS.items():
        ...
        if not lang_code:
            continue
        for topic in topics:
            if (lang, topic["key"]) not in generated:
                continue
            units.append({
                "unit_id":       f"grammar:{topic['lesson_id']}",
                "module":        "grammar",
                "source_lesson": lang_code,
                "source_item":   None,
                "level":         topic["level"],
                "topic":         f"{topic['title']} ({topic['gloss_en']})",
            })
    return units
...
def seed_target_grammar() -> list[dict]:
    """
    engine.target_grammar_paths's roadmap (LINGUISTIC_AUDIT.md section 1 /
    CLAUDE.md 2026-08-23) -- topics genuine to one specific target language,
    generated by scripts/generate_target_grammar_drills.py into
    data/target_grammar_drills.csv. Only topics that actually have generated
    sentences are seeded (a topic in the registry with no CSV rows yet --
    generation still running, or not started -- would otherwise create a
    content_units row the app can't actually serve anything for).

    source_lesson=<lang_code> locks each unit to its one target language via
    engine.recommender._candidates()'s existing "reading" mechanism -- see
    schema.sql's content_units comment.
    """
    drills_path = DATA_DIR / "target_grammar_drills.csv"
    if not drills_path.exists():
        return []
    ...
    for lang, topics in TARGET_GRAMMAR_PATHS.items():
        ...
        for topic in topics:
            if (lang, topic["key"]) not in generated:
                continue
            units.append({
                "unit_id":       f"target_grammar:{lang_code}:{topic['key']}",
                "module":        "target_grammar",
                "source_lesson": lang_code,
                "source_item":   None,
                "level":         topic["level"],
                "topic":         topic["gloss_en"],
            })
    return units
...
def main() -> None:
    ...
    all_units = (seed_grammar() + seed_target_grammar_lessons() + seed_vocab()
                 + seed_cefrj_vocab() + seed_reading() + seed_target_grammar())

    # Historical note, no longer true -- do NOT reintroduce a blanket
    # DELETE FROM content_units here. It was added 2026-08-21 on the
    # assumption that content_units is a pure catalog with no dependent
    # user state, safe to fully clear and reseed (the vocab/phrasebook
    # split had just orphaned the old "vocab:Hobbies:474"-style rows for
    # the 25 non-Word-Bank sheets). That assumption broke as soon as real
    # students accumulated real srs_state rows against real unit_ids:
    # srs_state.unit_id has a FOREIGN KEY REFERENCES content_units(unit_id)
    # with no ON DELETE CASCADE (schema.sql) -- confirmed the hard way
    # 2026-08-23, this DELETE failed with IntegrityError against a real
    # student's real "reading:es:1" progress, which is exactly the outcome
    # you want from a foreign key here, not something to work around.
    # Upsert-only can leave a genuinely renamed/retired unit_id's old row
    # behind (it stops being produced by seed_*() but nothing removes it) --
    # a real but separate cleanup problem from the one this comment used to
    # solve. Leaving it as future manual/targeted cleanup (only removing a
    # specific stale unit_id, never a blanket wipe) is the safe tradeoff:
    # inert clutter in the catalog vs. an IntegrityError -- or worse, a
    # silent CASCADE -- against real student progress.
    ...
    print(f"Seeded {len(all_units)} content_units "
          ...
          f"target_grammar={sum(1 for u in all_units if u['module']=='target_grammar')}).")
