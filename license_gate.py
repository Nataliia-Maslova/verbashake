...
def check_and_gate() -> bool:
    ...
    Returns True  → license is valid (or licensing is disabled), proceed normally.
    Returns False → license gate is being shown, stop rendering.

    The gate is INACTIVE when LEMON_API_KEY is not set in .streamlit/secrets.toml
    (development / pre-launch mode).
    """
    # Skip gate entirely if no API key is configured
    try:
